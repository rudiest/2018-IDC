...
class smartGridKendo_SSP extends SM_module {
    ...
    function auto_getSQL($SQL) {
        ...
        $ssp_sql_q_filter = $this->build_q_filter();
        
        if (strlen(trim($ssp_sql_filter))>0) {
            $ssp_sql_filter .= " ".$ssp_sql_q_filter;
        }
        elseif (strlen(trim($ssp_sql_q_filter))>0) {
            $ssp_sql_filter = " where ".$ssp_sql_q_filter;
        }

        $sspSQL = " select * from ( ".$SQL." limit 18446744073709551615 ) sspTemp ".$ssp_sql_filter." ".$ssp_sql_sort;

        return $sspSQL;

    }
    
    function build_q_filter() {
        ...
        if ((strlen(trim($ssp_filter))> 0) && (strtolower(trim($ssp_filter)) != 'undefined') && ($_GET['wSQL'] == 1)) {
