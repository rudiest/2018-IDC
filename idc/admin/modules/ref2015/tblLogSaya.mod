...
class tblLogSaya extends SM_module {

     function moduleConfig() {

        /* Load template */
        $this->useTemplate('defTpt','/ref2015/tblLogSaya');
    ...
    function buildGrid($tmp = '') {
        ...
        $SQL  = " select   date_format(act_time,'%Y-%m-%d') as tglaction,
                           ...
                           act_id_memo
                  from     log_activity
                  where    act_id_user = '".$member['idxNum']."'
        ...
        $sgkMod->directive['element_title'] = 'Daftar Aktifitas Akun '.$member['firstName'];
