  ...
  **/
  ...
  class functionDB extends SM_module {
    ...
    function dbupd($table, $columnNameAndValue, $whereclause, $cleanup = 1) {
        ...
        $SQLFetch = "select ".$colname." from ".$table." where ".$whereclause." ";
        $rhData = $this->dbH->query($SQLFetch);
        $this->dberror_show($rhData, $SQLFetch);
        $prevData = '';
        while ($rrData = $rhData->fetch()) {
            $prevData .= implode('|-mpv/=|',$rrData)."\r\n";
        }
    ...
    function dbdelete($table, $whereclause, $memo = '', $cleanup = 1) {
        ...
        $SQLFetch = "select * from ".$table." where ".$whereclause." ";
        $rhData = $this->dbH->query($SQLFetch);
        $this->dberror_show($rhData, $SQLFetch);
        $prevData = '';
        while ($rrData = $rhData->fetch()) {
    ...
    function encap_string($value, $escapechar = 0, $usequote = 1) {
