...
class smartGridKendo_Ext extends SM_module {
    ...
    function dataPreparation($gridSQL = '', $gridConfig) {
        ...
        $gridFilter = $json->decode(stripslashes($_POST['gF']));
        $gridGroup = $json->decode(stripslashes($_POST['gG']));
        $gridSort = $json->decode(stripslashes($_POST['gS']));
        ...
        if (strlen(trim($groupSQL)) > 0) {
            ...
            if (strlen(trim($whereSQL)) > 0) {
                $finGroupSQL .= " where ".trim($whereSQL);
            }
    ...
    function buildPdf($gridSQL = '', $gridConfig) {
        ...
        $html .= '<tr>';
        $html .= ' <td align="left" valign="top" width="80"><img src="../../sysapp/appimage/logo_top_print.jpg"></td>';
        ...
        $filename = './temp/tmpTemp/sapa_'.$_GET['option'].'_'.$member['member_id'].'_'.time().'.html';
    ...
    function buildXls($gridSQL = '', $gridConfig) {
        ...
        $filename = './temp/tmpXls/sapa_'.$_GET['option'].'_'.$member['member_id'].'_'.time().'.xls';
        ...
        $worksheet[$wCtr] =& $workbook->add_worksheet('SAPA Report');

        // create header report
        $worksheet[$wCtr]->write_string(0, 0, $gridConfig['title'],$formatHD1);
        $worksheet[$wCtr]->write_string(1, 0, 'SAPA Excel Generator, generated '.date('D, d M Y H:i:s'),$formatHD2);
